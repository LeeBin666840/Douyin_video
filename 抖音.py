'''
欢迎大家来到顾老师直播间
一个专注于编程知识分享技术交流为主的学习直播
喜欢编程想往程序员行业发展的朋友点点关注
新粉丝关注后课领取粉丝福利
'''
import time
...
一、明确需求：
- 目标：抖音高清无水印视频采集 视频链接！
- 网址：https://www.douyin.com/user/MS4wLjABAAAA1Req4TY_Zja8pcq_rjsoR9uOc1p2QVXT923ShiSdhrw?modal_id=7252552632554835234

f12 -> 网络network -> 刷新网页 -> media
搜索参数：
aWk6OTo0Ojw6NmhkPDk7OEBpanRrZjs6Zm1nZDMzNGkzM0BgMmIyMDQtNTAxY2M2NjYxYSMwZnJucjRnNHFgLS1kLWFzcw
----------------批量采集数据-------------------
①爬虫采集  过平台加密   js逆向
x-bougs 解决他 魔改加密算法  3k-5k 
mstoken   可以理解为常量
ac_signature  cookie加密的  
...
# 单个视频获取
import requests  # 调用爬虫功能
# 视频链接
链接 = '视频'
# 对链接进行请求  得到
数据 = requests.get(链接)
# 保存处理
open('美女.mp4', 'wb').write(数据.content)
...
https://www.douyin.com/user/MS4wLjABAAAA3Edl4dvMxf-9uaJf8Zi8pt6RDBKQPPUzczKMMtwgInzWsy9UP1MoLf9qFXUwTE13?modal_id=7114231589143416079
...
for li in lis:
    ...
    # print(video_info)
    # 解码
    # video_data = requests.utils.unquote(video_info)
    video_data = parse.unquote(video_info)
    # print(video_data)
    # 转格式 json str变成一个dict  双胞胎！！
